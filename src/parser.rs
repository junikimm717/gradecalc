...
pub fn read_section() -> Result<Vec<categories::Category>, ()> {
    ...
    loop {
        ...
        let v: Vec<String> = line.split_whitespace().map(|x| String::from(x)).collect();
        if v.len() == 0 {
            continue;
        }
        ...
        match cmd {
            "cat" | "category" | "c" => {
                ...
                if v.len() != 3 {
                    eprintln!(
                        "Error on line {}! Does not have the appropriate number of arguments.",
                        line_number
                    );
            ...
            "score" | "s" | "sc" => {
                ...
                if res.len() == 0 {
                    eprintln!(
                        "Error on line {}! Unclear which category the following score belongs to",
                        line_number
                    );
                    eprintln!("line {}: {:?}", line_number, &line);
                    return Err(());
                ...
                if v.len() == 2 {
                    res[ind]
                        .scores
                        .push(categories::Score::Percentage(parse_or_fail!(v[1])))
                } else if v.len() == 3 {
                    res[ind].scores.push(categories::Score::Points(
                        parse_or_fail!(v[1]),
                        parse_or_fail!(v[2]),
                    ))
                } else {
                    eprintln!(
                        "Error on line {}! score does not have enough arguments.",
                        line_number
                    );
                    eprintln!("line {}: {:?}", line_number, &line);
                    return Err(());
            ...
            u => {
                eprintln!(
                    "Error on line {}! {} is an undefined command",
                    line_number, u
                );
                eprintln!("line {}: {:?}", line_number, &line);
                return Err(());
