...
impl Category {
    ...
    pub fn avg(&self) -> Result<f64, ()> {
        if self.all_points() {
            return Ok(self.scores.iter().fold(0 as f64, |acc, next| {
                acc + match next {
                    Score::Points(a, _b) => a,
                    Score::Percentage(_a) => panic!("imposs"),
                }
            }) / self.scores.iter().fold(0 as f64, |acc, next| {
                acc + match next {
                    Score::Points(_a, b) => b,
                    Score::Percentage(_a) => panic!("imposs"),
                }
            ...
            return Ok(it.fold(0 as f64, |acc, next| {
                acc + match next {
            ...
            eprintln!(
                "Error! category {} must consist of scores that are all
            numbers or all based on points (do not mix and match them)",
                self.name
            );
            Err(())
        }
